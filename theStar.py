...
class Paint(object):
    ...
    def __init__(self):
        ...
        self.root = Tk()    
        self.root.protocol("WM_DELETE_WINDOW", lambda f: self.destroy() )
...
class EmbeddedNeuron(EmbeddedCanvas):
    ...
    def drawFill(self):
        self.undrawFill()
        fillRatio=180*min(max((self.neuron.voltage + self.neuron.sumInputs) / self.neuron.threshold,0),1)
...
class EmbeddedSynapse(EmbeddedCanvas):
    ...
    def redraw(self):
        ...
        if self.synapse.weight<0:
            R=int(EmbeddedSynapse.END_CIRCLE_RADIUS*2)
            SL=((self.NTo.centerX-self.NFrom.centerX)**2+(self.NTo.centerY-self.NFrom.centerY)**2)**0.5
            vX=((self.NTo.centerX-self.NFrom.centerX)/SL)
            vY=((self.NTo.centerY-self.NFrom.centerY)/SL)
            p1y=vX*R
            p1x=(-1*vY*p1y)/vX
            p2y=-1*vX*R
            p2x=(-1*vY*p2y)/vX
            ...
            self.shapeBulb=self.canvas.create_line(p1x, p1y, p2x, p2y,\
                                           fill=self.NFrom.mainColor, width=2)
        else:
    ...
    def drawPulse(self,t, wdt):
        ...
        temp=self.canvas.create_line(x1, y1, x2, y2,\
                                           fill="red", width=wdt)
        if x2==newEndX and y2==newEndY and self.synapse.weight>0:
